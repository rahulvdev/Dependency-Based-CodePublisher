...
void showUsage()
{
  ...
  out << "\n ****-Dependancy Analysis XML file path is ../../XMLFiles/dep.xml";
  out << "\n ****-Strongly Connected Components XML file path is ../../XMLFiles/scc.xml";
  out << "\n\n";
  std::cout << out.str();
  //Rslt::write(out.str());
  //Rslt::flush();
...
{
	std::cout << "------------------------------------------------------------------------------------------------------\n";
	std::cout << "REQUIREMENT 8:Command line processing to define files to publish by specifying path and file patterns\n";
	std::cout << "------------------------------------------------------------------------------------------------------\n";
	std::cout << "\n***Path to all files required for analysis is provided\n";
	std::cout << "\n***File patterns for analysis for ex. *.cpp,*css *.h etc are provided \n";
	std::cout << "\n***Path for writing XML files i.e Dependancy Table and Strong Component content is provided\n";
  if (argc < 2)
  {
    showUsage();
    return false;
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "--------------------------------------------------------------------------------------------------------\n";
  std::cout << "REQUIREMENT 10:Automated unit test suite that demonstrates you meet all the requirements of this project\n";
  std::cout << "--------------------------------------------------------------------------------------------------------\n";
  ...
  try {
	  ...
	  //Start Type Analysis
	  TypeAnalysis tb;
	  tb.startTypeAnalysis();

	  //Start Dependancy Analysis
	  DependancyAnalysis dp(exec.depXmlPath,50);
	  for (File file : allsubfiles) {
	  dp.startDependencyAnalysis(tb,file);
  }
	  std::cout << "\n";
	  std::cout << "\n";
	  ...
	  cPub.fileCollector(allsubfiles,lineMapInst);
  }
