...
CodePublisher::CodePublisher(DependancyAnalysis& dpObj) :dpAnal(dpObj) {
	std::cout << "\n\n";
	std::cout << "------------------------------------------------------------\n";
	std::cout << "HTML files are created in path ..\\..\\FilesToBePublished\\HTML \n";
	std::cout << "------------------------------------------------------------\n";
	std::cout << "----------------------------------------------------------------------------------------------------------\n";
...
void CodePublisher::fileCollector(std::vector<std::string> listOfFiles, lineNoMap& lMap)
{	
	std::ofstream ostIndex;
	ostIndex.open(htmlFilePath+"index.htm");
	std::cout << "-------------------------------------------------------------------------------------------------\n";
	std::cout << "REQUIREMENT 9: All important packages are published in ..\\..\\FilesToBePublished\\HTML\\index.htm file \n";
	std::cout << "-------------------------------------------------------------------------------------------------\n";
	...
	if (listOfFiles.size() != 0) {
		for (std::string file : listOfFiles) {
			generateHTMlfile(file,lMap,ostIndex);
		}
		ostIndex << "</body>\n";
		ostIndex << "</html>";
		ostIndex.close();
		openFilesInBrowser("index");
	}
}
...
//Eliminates fully qualified structure
	...
	void CodePublisher::generateHTMlfile(std::string file,lineNoMap& lMap,std::ofstream& ostIndex)
	{
		...
		writeToHTMLfile(file,fileName,lMap);
	}
	...
	void CodePublisher::writeToHTMLfile(std::string fileToRead, std::string htmlFileName,lineNoMap& lMap)
	{	int counter = 1;
		...
		if (myfile.is_open())
		{
			...
			printDependancy(myfile, htmlFileName, dependantFileVect);
			generateTags(myfile);
...
void CodePublisher::printDependancy(std::ofstream& ost,std::string fileNameOnly,std::vector<std::string> dependancyVect)
	{
...
void CodePublisher::openFilesInBrowser(std::string htmlFileName) {
	std::string path = "file:///" + FileSystem::Path::getFullFileSpec("..\\..\\Filestobepublished\\HTML\\" + htmlFileName + ".htm");
